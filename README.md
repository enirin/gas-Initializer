...
このエラーは CI/CD ワークフロー実行時に `.clasprc.json` の JSON 解析に失敗している場合に発生します。

**原因:**
- 秘密情報が複数行に分かれて登録されている
- 秘密情報にホワイトスペースや改行が含まれている
...
1. 秘密情報を再登録してください：
   ...
   ```
   その後、`init-gas-project.ps1` の Step 8 を実行するか、以下で登録：
   ```powershell
   register-gas-environment-secrets.ps1
   ```
